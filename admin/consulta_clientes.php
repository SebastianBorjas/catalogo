...
if ($buscarEmpresa->num_rows > 0)
{
	...
	'<table class="table">
		<tr class="bg-primary">
			<td style="background-color: #221F43; color: #ffffff; text-align: center;">ID CLIENTE</td>
			<td style="background-color: #221F43; color: #ffffff; text-align: center;">SECTOR AL QUE PERTENECE</td>
			<td style="background-color: #221F43; color: #ffffff; text-align: center;">LOGO</td>
	...
	while($filasCliente= $buscarEmpresa->fetch_assoc())
	{
		...
		'
		<tr>
		<td class="text-center">'.$filasCliente['idCliente'].'</td>
		<td class="text-center">'.$filasCliente['nombre'].'</td>
		<td class="text-center"><img  style="width: 100px; height: 100px;" src=../ImagenesEmpresas/Clientes/'.$filasCliente['logo'].' ></td>
