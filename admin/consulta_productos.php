...
if(isset($_POST['productos']))
{
	...
	$query="SELECT * FROM Producto WHERE 
		descripcion LIKE '%".$q."%' or idEmpresa LIKE '".$q."%'";
...
if ($buscarEmpresa->num_rows > 0)
{
	...
	'<table class="table">
		<tr class="bg-primary">
			<td style="background-color: #221F43; color: #ffffff; text-align: center;">ID PRODUCTO</td>
			<td style="background-color: #221F43; color: #ffffff; text-align: center;">DESCRIPCIÓN</td>
	...
	while($filasProducto= $buscarEmpresa->fetch_assoc())
	{
		$tabla.=
		'
		<tr>
		<td class="text-center">'.$filasProducto['idProducto'].'</td>
		<td class="text-center">'.$filasProducto['descripcion'].'</td>
		<td class="text-center"><img  style="width: 100px; height: 100px;" src=../ImagenesEmpresas/Productos/'.$filasProducto['imagen'].' ></td>
		<td class="text-center">
			<a class="btn btn-info" id="Validar" data-id="'.$filasProducto["idEmpresa"].'" style="background-color: #D72525;" ><font color="WHITE"> GENERAR PDF <span class="glyphicon glyphicon-download"></span></a>    
		</td>
